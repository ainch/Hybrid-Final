#include "cuda_Field_SpeedTest.cuh"
// FOR Field method 4
...
void PCG_Laplace_TEST(){
    ...
    if(Lap_Field_Solver_Flag == 0){// [CPU] Conjugate Gradient 
        ...
        *dot_result = 0.0;
        ...
        if(Lap_Field_Solver_Save) Field_Laplace_Solution_Save(Namebuf,CPUsol);
    }else if(Lap_Field_Solver_Flag == 7){// [GPU] [IChol] Preconditioned Conjugate Gradient
        void *buffer = NULL;
        int stmp = 0;
        size_t bufferSize = 0;
        ...
        *dot_result = 0.0;
	    ...
	    checkCudaErrors(cusparseSetMatFillMode(descr_A, CUSPARSE_FILL_MODE_LOWER));
	    checkCudaErrors(cusparseSetMatDiagType(descr_A, CUSPARSE_DIAG_TYPE_NON_UNIT));
	    checkCudaErrors(cusparseSetMatIndexBase(descr_A, CUSPARSE_INDEX_BASE_ONE));
        /* Create L factor descriptor */
        cusparseMatDescr_t descr_L = NULL;
        cusparseStatus = cusparseCreateMatDescr(&descr_L);    
        ...
        /* Allocate bufferSize */
        checkCudaErrors(cusparseScsric02_bufferSize(cusparseHandle, A_size, 5*A_size, descr_L, dev_A, dev_Ai, dev_Aj, infoIC, &stmp));
        bufferSize = stmp;
        checkCudaErrors(cudaMalloc(&buffer, bufferSize));
        /* Perform analysis for IC(0) */
        checkCudaErrors(cusparseScsric02_analysis(
            cusparseHandle, A_size, 5*A_size, descr_L, dev_A, dev_Ai, dev_Aj, infoIC,
        ...
        checkCudaErrors(cusparseScsric02(
            cusparseHandle,  A_size, 5*A_size, descr_L,dev_L, dev_Ai, dev_Aj, infoIC,
            CUSPARSE_SOLVE_POLICY_USE_LEVEL, buffer));
        printf("Complete!!\n");
	    ...
	    checkCudaErrors(cusparseCreateCsr(&smat_A,  N, N, nz, dev_Ai, dev_Aj, dev_A, CUSPARSE_INDEX_32I,
		    CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ONE, CUDA_R_64F));
	    cusparseDnVecDescr_t dvec_p;
	    checkCudaErrors(cusparseCreateDnVec(&dvec_p, N, dev_P, CUDA_R_64F));
	    cusparseDnVecDescr_t dvec_q;
	    checkCudaErrors(cusparseCreateDnVec(&dvec_q, N, dev_AP, CUDA_R_64F));
	    cusparseDnVecDescr_t dvec_x;
	    checkCudaErrors(cusparseCreateDnVec(&dvec_x, N, dev_X, CUDA_R_64F));
	    // Compute buffer size in solving linear system
	    checkCudaErrors(cusparseScsrsv2_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, 
		    N, nz, descr_L,dev_L, dev_Ai, dev_Aj, info_L, &i_temp_buf_size));
	    u_temp_buf_size = i_temp_buf_size;
	    checkCudaErrors(cusparseScsrsv2_bufferSize(cusparseHandle, CUSPARSE_OPERATION_TRANSPOSE,
		    N, nz, descr_L,dev_L, dev_Ai, dev_Aj, info_U, &i_temp_buf_size));
	    ...
	    checkCudaErrors(cusparseSpMV_bufferSize(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, smat_A,
	    ...
	    if (u_temp_buf_size > buf_size){
		    ...
		    cudaFree(d_buf);
		    checkCudaErrors(cudaMalloc(&d_buf, buf_size));
	    }
        /* --- Initialize variables --- */
	    // analysis phase
	    checkCudaErrors(cusparseScsrsv2_analysis(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE,
		    N, nz, descr_L, dev_L, dev_Ai, dev_Aj, info_L, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_buf));
	    checkCudaErrors(cusparseScsrsv2_analysis(cusparseHandle, CUSPARSE_OPERATION_TRANSPOSE,
		    N, nz, descr_L, dev_L, dev_Ai, dev_Aj, info_U, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_buf));
        const int max_iter = 810;
        int iter = 0;
        float n_one;
        float rho, rho_t;
        float rTr,beta,alpha,pTq;
        for (k = 0; k < CondNUMR; k++) {
            checkCudaErrors(cudaMemcpy(dev_PCG_const, Host_PCG_const,sizeof(DPS_Const), cudaMemcpyHostToDevice));
            checkCudaErrors(cudaMemcpy(dev_b, cond_b[k], A_size * sizeof(float),cudaMemcpyHostToDevice));
            checkCudaErrors(cudaMemcpy(dev_R, dev_b, A_size * sizeof(float),cudaMemcpyDeviceToDevice));
		    ...
		    //     q = A*x
		    //     r0 = -q + b
		    checkCudaErrors(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, smat_A,
			    dvec_x, &zero, dvec_q, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, d_buf));
		    n_one = -1;
		    checkCudaErrors(cublasSaxpy(cublasHandle, N, &n_one, dev_Y, 1, dev_R, 1));
	        /* --- Perform conjugate gradient --- */
	        // Solving linear system
	        for (iter = 0; iter < max_iter; ++iter){
		        // if ||rk|| < tolerance
		        checkCudaErrors(cublasSnrm2(cublasHandle, N, dev_R, 1, &rTr));
		        if(iter<10) printf("Iteration = %d, rsold = %g\n",iter,rTr);
		        if (rTr < PCGtol2){
			        break;
		        }
		        // Solve L*y = rk
		        checkCudaErrors(cusparseScsrsv2_solve(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, &one,
			        descr_L, dev_L, dev_Ai, dev_Aj, info_L, dev_R, dev_Y, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_buf));
		        // Solve L^T*zk = y
		        checkCudaErrors(cusparseScsrsv2_solve(cusparseHandle, CUSPARSE_OPERATION_TRANSPOSE, N, nz, &one,
			        descr_L, dev_L, dev_Ai, dev_Aj, info_U, dev_Y, dev_Z, CUSPARSE_SOLVE_POLICY_USE_LEVEL, d_buf));
		        // rho_t = r{k-1} * z{k-1}
                rho_t = rho;  
		        // rho = rk * zk
		        checkCudaErrors(cublasSdot(cublasHandle, N, dev_R, 1, dev_Z, 1, &rho));
		        if (iter == 0){
			        // pk = zk
			        checkCudaErrors(cublasScopy(cublasHandle, N, dev_Z, 1, dev_P, 1));
		        }else{
			        // beta = (rk*zk) / (r{k-1}*z{k-1})
			        beta = rho / rho_t;
			        // pk = zk + beta*p{k-1}
			        checkCudaErrors(cublasSscal(cublasHandle, N, &beta, dev_P, 1));
			        checkCudaErrors(cublasSaxpy(cublasHandle, N, &one, dev_Z, 1, dev_P, 1));
		        }
                // q = A*pk
		        checkCudaErrors(cusparseSpMV(cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, &one, smat_A,
			        dvec_p, &zero, dvec_q, CUDA_R_64F, CUSPARSE_MV_ALG_DEFAULT, d_buf));
		        // alpha = (rk*zk) / (pk*q)
		        checkCudaErrors(cublasSdot(cublasHandle, N, dev_P, 1, dev_AP, 1, &pTq));
		        alpha = rho / pTq;
		        // x{k+1} = xk + alpha*pk
		        checkCudaErrors(cublasSaxpy(cublasHandle, N, &alpha, dev_P, 1, dev_X, 1));
		        // r{k+1} = rk - alpha*q 
		        float n_alpha = -alpha;
		        checkCudaErrors(cublasSaxpy(cublasHandle, N, &n_alpha, dev_AP, 1, dev_R, 1));
	        }
            printf("Iteration = %d, rsold = %g\n",iter,rTr);
            cudaEventRecord( stop, 0 ); cudaEventSynchronize( stop );
	        cudaEventElapsedTime( &gputime, start, stop );
	        cudaEventDestroy( start );cudaEventDestroy( stop );
            printf("Solution %d",k);
        ...
        if(Lap_Field_Solver_Save) Field_Laplace_Solution_Save(Namebuf,CPUsol);
        exit(1);
    }else if(Lap_Field_Solver_Flag == 8){// [GPU] [ILU] Preconditioned Conjugate Gradient
        int N,nz;
        ...
        N = A_size;
        nz = 5*A_size;
        Field_Method8_Initial();
        // Initial Setting
        /* Description of the A matrix */
        cusparseMatDescr_t descr = 0;
        checkCudaErrors(cusparseCreateMatDescr(&descr));
        checkCudaErrors(cusparseSetMatType(descr, CUSPARSE_MATRIX_TYPE_GENERAL));
        checkCudaErrors(cusparseSetMatIndexBase(descr, CUSPARSE_INDEX_BASE_ONE));
        cusparseSpMatDescr_t matA = NULL;
        checkCudaErrors(cusparseCreateCsr(&matA, N, N, nz, dev_Ai, dev_Aj, dev_A, CUSPARSE_INDEX_32I,
        CUSPARSE_INDEX_32I, CUSPARSE_INDEX_BASE_ONE, CUDA_R_32F));
        cusparseDnVecDescr_t vecp = NULL;
        checkCudaErrors(cusparseCreateDnVec(&vecp, N, dev_P, CUDA_R_32F));
        cusparseDnVecDescr_t vecomega = NULL;
        checkCudaErrors(cusparseCreateDnVec(&vecomega, N, dev_AP, CUDA_R_32F));
        ...
        checkCudaErrors(cusparseSetMatIndexBase(descrL, CUSPARSE_INDEX_BASE_ONE));
        ...
        checkCudaErrors(cusparseScsrilu02_bufferSize(
            cusparseHandle, N, nz, descr, dev_A, dev_Ai, dev_Aj, infoILU, &stmp));
        if (stmp > bufferSize) {
            bufferSize = stmp;
        }
        checkCudaErrors(cusparseScsrsv2_bufferSize(
            cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, descrL, dev_A, dev_Ai, dev_Aj, infoL, &stmp));
        if (stmp > bufferSize) {
            bufferSize = stmp;
        }
        checkCudaErrors(cusparseScsrsv2_bufferSize(
            cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, descrU, dev_A, dev_Ai, dev_Aj, infoU, &stmp));
        if (stmp > bufferSize) {
            bufferSize = stmp;
        }
        checkCudaErrors(cudaMalloc(&buffer, bufferSize));
       ...
       "Matrix Computations 3rd ed.", Algorithm 10.3.1  */
        ...
        checkCudaErrors(cusparseScsrilu02_analysis(
            cusparseHandle, N, nz, descr,dev_A, dev_Ai, dev_Aj, infoILU,
            CUSPARSE_SOLVE_POLICY_USE_LEVEL, buffer));
        /* Copy A data to ILU(0) vals as input*/
        // Make a Preconditioner matrix
        checkCudaErrors(cudaMalloc((void**) &dev_L, nz* sizeof(float)));
        checkCudaErrors(cudaMemset((void *) dev_L, 0.0, nz* sizeof(float)));
        checkCudaErrors(cudaMemcpy(dev_L, dev_A, nz*sizeof(float), cudaMemcpyDeviceToDevice));
        /* generate the ILU(0) factors */
        checkCudaErrors(cusparseScsrilu02(
            cusparseHandle, N, nz, descr, dev_L, dev_Ai, dev_Aj, infoILU,
            CUSPARSE_SOLVE_POLICY_USE_LEVEL, buffer));
        /* perform triangular solve analysis */
        checkCudaErrors(cusparseScsrsv2_analysis(
            cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, descrL,
            dev_L, dev_Ai, dev_Aj, infoL, CUSPARSE_SOLVE_POLICY_USE_LEVEL,
            buffer));
        checkCudaErrors(cusparseScsrsv2_analysis(
            cusparseHandle, CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, descrU,
            dev_L, dev_Ai, dev_Aj, infoU, CUSPARSE_SOLVE_POLICY_USE_LEVEL,
            buffer));
        int iter;
        ...
        for (k = 0; k < CondNUMR; k++) {
            ...
            /* PCG start */
            iter = 0;
            checkCudaErrors(cublasSdot(cublasHandle, N, dev_R, 1, dev_R, 1, &r1));
            while (r1 > PCGtol2 && iter <= max_iter)
            {
                // preconditioner application: d_zm1 = U^-1 L^-1 dev_R
                checkCudaErrors(cusparseScsrsv2_solve(cusparseHandle,
                    CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, &floatone, descrL,
                    dev_L, dev_Ai, dev_Aj, infoL, dev_R, dev_Y,
                    CUSPARSE_SOLVE_POLICY_USE_LEVEL, buffer));
                checkCudaErrors(cusparseScsrsv2_solve(cusparseHandle,
                    CUSPARSE_OPERATION_NON_TRANSPOSE, N, nz, &floatone, descrU,
                    dev_L, dev_Ai, dev_Aj, infoU, dev_Y, dev_Z,
                    CUSPARSE_SOLVE_POLICY_USE_LEVEL, buffer));
                iter++;
                if (iter == 1){
                    checkCudaErrors(cublasScopy(cublasHandle, N, dev_Z, 1, dev_P, 1));
                }else{
                    checkCudaErrors(cublasSdot(cublasHandle, N, dev_R, 1, dev_Z, 1, &numerator));
                    checkCudaErrors(cublasSdot(cublasHandle, N, d_rm2, 1, d_zm2, 1, &denominator));
                    beta = numerator/denominator;
                    checkCudaErrors(cublasSscal(cublasHandle, N, &beta, dev_P, 1));
                    checkCudaErrors(cublasSaxpy(cublasHandle, N, &floatone, dev_Z, 1, dev_P, 1));
                }
                checkCudaErrors(cusparseSpMV(
                    ...
                    buffer));
                checkCudaErrors(cublasSdot(cublasHandle, N, dev_R, 1, dev_Z, 1, &numerator));
                checkCudaErrors(cublasSdot(cublasHandle, N, dev_P, 1, dev_AP, 1, &denominator));
                alpha = numerator / denominator;
                checkCudaErrors(cublasSaxpy(cublasHandle, N, &alpha, dev_P, 1, dev_X, 1));
                checkCudaErrors(cublasScopy(cublasHandle, N, dev_R, 1, d_rm2, 1));
                checkCudaErrors(cublasScopy(cublasHandle, N, dev_Z, 1, d_zm2, 1));
                nalpha = -alpha;
                checkCudaErrors(cublasSaxpy(cublasHandle, N, &nalpha, dev_AP, 1, dev_R, 1));
                checkCudaErrors(cublasSdot(cublasHandle, N, dev_R, 1, dev_R, 1, &r1));
            }
            /* PCG End */
            printf("Iteration = %d, rsold = %g\n",iter,r1);
